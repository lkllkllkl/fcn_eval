...
from convert_fcn_dataset import image2label
...
def input_tensor(image_path, upsample_factor_for_whole_net=32):
    ...
    target_input_size_factor = tf.ceil(tf.div(tf.to_float(original_shape), tf.to_float(upsample_factor_for_whole_net)))
    target_input_size = tf.to_int32(tf.multiply(target_input_size_factor, upsample_factor_for_whole_net))
    padding_size = (target_input_size - original_shape) // 2
...
def lable_input(lable_path, upsample_factor_for_whole_net=32):
    img_label = cv2.imread(lable_path)
    image = image2label(img_label)
    image_float = tf.to_float(image, name='ToFloat')
    shape = tf.shape(image)
    label = tf.reshape(image_float, [shape[0], shape[1], 1])
    ...
    label = tf.expand_dims(annotation_tensor_paded, 0)
